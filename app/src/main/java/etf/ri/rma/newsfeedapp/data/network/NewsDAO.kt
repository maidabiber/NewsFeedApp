...
class NewsDAO {
    ...
    companion object {
       ...
       isFeatured = false,
        ...
        category = "sports",
       // url = "https://www.espn.com/nba/story/_/id/45460015/2025-nba-finals-game-1-tunnel-fashion-thunder-pacers",
        isFeatured = false,
        source = "espn.com",
        publishedDate = "06-06-2025"
        ),
        ...
        category = "sports",
       // url = "https://www.espn.com/college-football/story/_/id/45460370/twin-sister-5-star-alabama-freshman-qb-keelon-russell-dies",
        isFeatured = false,
        source = "espn.com",
        publishedDate = "06-06-2025"
        ),
       ...
       isFeatured = false,
        ...
        category = "business",
        //url = "https://www.cnbc.com/2025/06/05/were-lifting-our-price-target-on-broadcom-after-its-ai-business-impresses-once-again.html",
        isFeatured = false,
        source = "cnbc.com",
        publishedDate = "05-06-2025"
        ),
    ...
    fun popuniSaPocetnimVijestima() {
        runBlocking {  cacheMutex.withLock {
    ...
    fun postaviApiServis(servis: NewsApiService) {       apiService = servis
    }

    fun vratiMapiranuKategoriju(localCategory: String): String =
        mapiranje[localCategory]?.lowercase(Locale.ROOT) ?: "general"

    fun getNewsItemApiCategory(localCategory: String): String =
        mapiranje[localCategory]?.replaceFirstChar { it.titlecase(Locale.ROOT) } ?: "General"

    suspend fun getTopStoriesByCategory(localCategory: String, forceRefresh: Boolean = false): List<NewsItem> = withContext(Dispatchers.IO) {

        val posljednjiPoziv = lastApiCallTimes[localCategory] ?: 0L

        val tren = System.currentTimeMillis()
        val apiCat = vratiMapiranuKategoriju(localCategory)

        if (!forceRefresh && tren - posljednjiPoziv < CATEGORY_API_CALL_COOLDOWN) {
           return@withContext kesiraneVijesti.values.filter { it.isFeatured && it.category.equals(apiCat, true) }
        }

       return@withContext try {

            val resp = apiService.getNews(
                apiKey = apiKljuc,
                categories = vratiMapiranuKategoriju(localCategory),
                limit = 40
            )
            ...
            cacheMutex.withLock {
                val newIds = resp.data.map { it.uuid }.toSet()

                kesiraneVijesti.values.forEach {
                    if (it.isFeatured && it.category.equals(apiCat, true) && it.uuid !in newIds) {
                        kesiraneVijesti[it.uuid] = it.copy(isFeatured = false)
                    }
                }

                resp.data.forEach { apiArt ->

                    val item = apiArt.toNewsItem().copy(isFeatured = true, category = getNewsItemApiCategory(localCategory))
                    kesiraneVijesti[item.uuid] = item
                    noveIstaknuteVijesti += item
                }
            }
            lastApiCallTimes[localCategory] = tren
    ...
    suspend fun getSimilarStories(uuid: String): List<NewsItem> = withContext(Dispatchers.IO) {
        ...
        similarNewsMutex.withLock {
            ...
            return@withContext try {
                val response = apiService.getSimilarStories(uuid, apiKljuc)

                val result = response.data.map { apiArticle ->
                    apiArticle.toNewsItem().copy(isFeatured = false)
                }.take(2).also { similarNews ->
                    similarNews.forEach {
                        kesiraneVijesti[it.uuid] = it
                    }
                }

                kesSlicnihVijesti[uuid] = result
               result
            } catch (e: Exception) {
                emptyList()
            }
